...
# this code part is for flipout, there is one more below commented out for BBBBNN


def load_signal_data(dataset_dir: str) -> tuple:
    """Load train and test data from CSV and return as PyTorch tensors.
    ...
    -------
    tuple
        Train data, train labels, test data, test labels as tensors.
    """
    train_data = torch.tensor(
        pd.read_csv(os.path.join(dataset_dir, "X_train_processed.csv"))
        .fillna(0)
        .values,
        dtype=torch.float32,
    )
    test_data = torch.tensor(
        pd.read_csv(os.path.join(dataset_dir, "X_test_processed.csv")).fillna(0).values,
        dtype=torch.float32,
    )

    train_labels = torch.tensor(
        pd.read_csv(os.path.join(dataset_dir, "y_train.csv")).values.squeeze(),
        dtype=torch.float32,
    )
    test_labels = torch.tensor(
        pd.read_csv(os.path.join(dataset_dir, "y_test.csv")).values.squeeze(),
        dtype=torch.float32,
    )
...
def load_datasets(data_dir: str) -> tuple:
    """Load datasets as PyTorch TensorDataset instances.
    ...
    -------
    tuple
        Train and test datasets as TensorDataset.
    """
...
def make_performance_uncertainty_plot(
    y_true: np.array,
    y_pred: np.array,
    y_unc: np.array,
    y_axis_label: str,
    ...
    """Create a plot to show the relationship between uncertainty and model performance.
    ...
    performance_fn_args: dict
        Additional arguments for performance function.

    Returns
    -------
    plt.figure
        Performance vs Uncertainty plot.
    """
    y_unc = y_unc.squeeze()
    ...
    first_index = max(128, np.argwhere(sorted_labels != sorted_labels[0])[0][0])
    performances = []
    percentages = []
    ...
    for i in range(first_index + 1, len(sorted_uncertainties)):
        ...
        performances.append(np.mean(np.abs(selected_labels - selected_predictions)))
